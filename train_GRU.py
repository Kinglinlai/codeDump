...
class GRUForecast(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(hidden_size, num_features * OUTPUT_SEQ_LEN)

    def forward(self, x: Tensor) -> Tensor:
        # x: (B, F, 90) → (B, 90, F) for GRU
        x = x.transpose(1, 2)
        _, h_n = self.gru(x)          # h_n: (num_layers, B, H)
        h_last = h_n[-1]              # final layer’s hidden state (B, H)
        out = self.fc(h_last)         # (B, F*15)
        return out.view(x.size(0), -1, OUTPUT_SEQ_LEN)  # (B, F, 15)
...
def train(
    *,
    ...
    model = GRUForecast(num_features=len(FEATURE_COLUMNS)).to(device)
    ...
    # --------------------------------------------------------------------- #
    # Save
    out_path = save_dir / "weather_GRU.pt"
    torch.save(
        {
